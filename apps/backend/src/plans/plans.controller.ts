...
export class PlansController {
  ...
  constructor(private readonly planCacheService: PlanCacheService) {}
  ...
  @ApiQuery({
    ...
    required: false,
    type: 'boolean',
    description: 'Filter plans by test mode (true for test plans, false for production plans)'
  })
  @ApiQuery({
    name: 'tier',
    required: false,
  ...
  async getPlans(
    ...
    try {
      ...
      // Get all plans
      const allPlans = await this.planCacheService.getAllPlans();
      ...
      this.logger.log(`Returning ${plans.length} plans (dev mode: ${isDevMode})`);
  ...
  async getPlansByTier() {
    try {
      const tieredPlans = await this.planCacheService.getPlansGroupedByTier();
      const isDevMode = process.env.NODE_ENV !== 'production';
