...
export const RealtimeInsights: React.FC<RealtimeInsightsProps> = ({
  ...
  const occupiedTables = tableStatuses.filter(t => t.status === TableStatusType.Occupied).length;
  const availableTables = tableStatuses.filter(t => t.status === TableStatusType.Available).length;
  const needsAttentionTables = tableStatuses.filter(t => t.status === TableStatusType.NeedsAttention).length;
  ...
  const occupancyTrend = occupancyRate > 70 ? 'high' : occupancyRate > 40 ? 'medium' : 'low';

  // Calculate table efficiency
  const tableEfficiency = tableStatuses.map(table => {
    const efficiency = table.dailyMetrics.turnoverCount > 0
      ? table.dailyMetrics.totalRevenue / table.dailyMetrics.turnoverCount
      : 0;
    return { tableId: table.tableId, tableLabel: table.tableLabel, efficiency };
  }).sort((a, b) => b.efficiency - a.efficiency);

  const topPerformingTables = tableEfficiency.slice(0, 3);
  const underperformingTables = tableEfficiency.slice(-3).filter(t => t.efficiency > 0);
  ...
  const generateInsights = () => {
    ...
    if (occupancyRate > 85) {
      insights.push({
        ...
        message: 'Restaurant is near capacity. Consider preparing for wait times.',
    ...
    if (averageOrderValue > 50) {
      insights.push({
        ...
        message: `Average order value is $${averageOrderValue.toFixed(2)} - excellent performance!`,
    ...
    if (occupancyRate < 30 && new Date().getHours() > 18) {
      insights.push({
        ...
        message: 'Consider promotional activities to increase evening footfall.',
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              {occupancyTrend === 'high' && <TrendingUp className="w-3 h-3 text-green-600" />}
              {occupancyTrend === 'medium' && <Activity className="w-3 h-3 text-yellow-600" />}
              {occupancyTrend === 'low' && <TrendingDown className="w-3 h-3 text-red-600" />}
              <span className="text-xs text-muted-foreground capitalize">{occupancyTrend} traffic</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{insights?.averageWaitTime || 12}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{insights?.revenueRate || 85}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <h4 className="font-medium text-sm mb-2 text-green-700">Top Performers</h4>
            ...
            {underperformingTables.length > 0 && (
              <div>
                <h4 className="font-medium text-sm mb-2 text-orange-700">Needs Attention</h4>
                <div className="space-y-2">
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span>{occupiedTables}/{totalTables}</span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="p-2 rounded-md bg-green-50">
                  <div className="font-semibold text-green-700">{availableTables}</div>
                  <div className="text-xs text-green-600">Available</div>
                </div>
                <div className="p-2 rounded-md bg-red-50">
                  <div className="font-semibold text-red-700">{occupiedTables}</div>
                  <div className="text-xs text-red-600">Occupied</div>
                </div>
                <div className="p-2 rounded-md bg-orange-50">
                  <div className="font-semibold text-orange-700">{needsAttentionTables}</div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 rounded-lg bg-muted/50">
              ...
              <div className="text-sm text-muted-foreground">Revenue per Table</div>
              <div className="text-xs text-muted-foreground mt-1">Active tables</div>
            </div>
            <div className="text-center p-4 rounded-lg bg-muted/50">
              ...
              <div className="text-xs text-muted-foreground mt-1">Current pace</div>
            </div>
          </div>
        </CardContent>
      </Card>
